firebase.auth().onAuthStateChanged(async function(user) {
  if(user) {
    ...
    let query = await db.collection('watched').where('userId','==',`${user.uid}`).get() //Not efficient to call a database within a loop because page noticeably loads one movie at a time. Moved query outside and added opacity after populating page
    ...
    for(let i = 0; i < watchList.length; i++){ //Add opacity class
      let movieId = watchList[i].data().movieId

      let target = document.querySelector(`.movie-${movieId}`)
      if(target){
